...
namespace ConsoleApplicationCpp
{
	...
	Bin::Bin(_hashedUnorderedOutcomePtrSet(std::shared_ptr) binOutcomes)
	...
	/// </summary>
	/// <returns></returns>
	std::unordered_set<std::shared_ptr<Outcome>, OutcomePointerHash, OutcomePointerEqual> Bin::GetOutcomes() const
	{
		return outcomes;
	}
}